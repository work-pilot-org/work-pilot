...
import { EmployeeResponse, OnboardingTaskResponse } from "@/types/hr";
...
export default function OnboardingPage() {
  ...
  const [tasks, setTasks] = useState<OnboardingTaskResponse[]>([]);
  ...
  const fetchEmployees = async () => {
    try {
      ...
      // Show newly invited/recent employees
      const recent = data.filter(e => e.is_active || e.invitation_status === 'PENDING');
      setEmployees(recent);
    } catch (err: unknown) {
  ...
  const fetchTasks = async (employeeId: string) => {
    try {
      const data = await hrRepository.getOnboardingTasks(employeeId);
      setTasks(data);
    } catch (err: unknown) {
  ...
  const handleAddTask = async () => {
    ...
    try {
      ...
      await hrRepository.createOnboardingTask({
  ...
  const handleToggleTask = async (task: OnboardingTaskResponse) => {
    try {
      await hrRepository.updateOnboardingTask(task.id, { is_completed: !task.is_completed });
      fetchTasks(selectedEmployee);
    } catch (err: unknown) {
  ...
  const handleDeleteTask = async (id: string) => {
    ...
    try {
      await hrRepository.deleteOnboardingTask(id);
      fetchTasks(selectedEmployee);
  ...
  return (
    <div className="p-8 max-w-5xl mx-auto space-y-8">
      <div>
        ...
        <p className="text-sm text-gray-500 mt-1">Manage onboarding checklists for employees.</p>
      </div>
        ...
        <div className="md:col-span-2 space-y-6">
          {!selectedEmployee ? (
            ...
            <div className="space-y-6 bg-white p-6 rounded-xl border shadow-sm">
              <h2 className="font-semibold text-xl">Onboarding Checklist</h2>
